...
@if($electiveBatch->program->agreement && !$electiveBatch->batchUser->accept_agreement)

<div class="row">
  <div class="col-12 mt-5">
    <div class="card p-5">
      {!! $electiveBatch->program->agreement->content !!}
...
{{ Form::open(array('route' => [Config::get('app.p_slug').'.my_electives.batches.accept_agreement', [$electiveBatch->program->id, $electiveBatch->id]], 'method' => 'post')) }}
          ...
          <div class="course_tabs">
            <nav>
              <div class="nav nav-tabs tab_crse justify-content-center" id="nav-tab" role="tablist">
                ...
                <a class="nav-item nav-link {{ $activeTab == 'feedback' ? 'active' : '' }}" id="nav-comments-feedback-tab" data-toggle="tab" href="#nav-comments-feedback" role="tab" aria-selected="false">@if($feedbacksCount < 3 && $electiveBatch->end_date < Carbon\Carbon::today()) Share Your Feedback @else View Feedback @endif</a>
              </div>
            </nav>						
          </div>
        </div>
          ...
          <div class="course_tab_content">
            ...
            <div class="tab-pane fade {{ $activeTab == 'journey' ? 'show active' : '' }}" id="nav-journey" role="tabpanel">
              <div class="_htg451">
                ...
                @if(count($electiveBatch->courseJourneys) > 0)
                <div class="container-fluid blue-bg">
                  <div class="container">


                  @foreach($electiveBatch->courseJourneys as $courseJourneyKey => $courseJourney)

                    @php($tail = 'full')

                    @if($courseJourneyKey == 0)
                      @php($tail = 'bottom')
                    @elseif(count($electiveBatch->courseJourneys) == $courseJourneyKey + 1)
                      @php($tail = 'top')
                    @else
                    ...
                    @if(count($electiveBatch->courseJourneys) != $courseJourneyKey + 1)
                    <!--line-->
                ...
                <div class="crse_content">
                  <div class="ui-accordion ui-widget ui-helper-reset">
                    <div class="ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom">
                      ...
                      @if(count($electiveBatch->program->resources))
                      @foreach($electiveBatch->program->resources as $resource)
              ...
              <div class="tab-pane fade {{ $activeTab == 'recordings' ? 'show active' : '' }}" id="nav-recordings" role="tabpanel">
                <div class="crse_content">
                  ...
                  @if(count($electiveBatch->sessions))
                    @foreach($electiveBatch->sessions as $session)
              ...
              <div class="tab-pane fade {{ $activeTab == 'assignments' ? 'show active' : '' }}" id="nav-assignments" role="tabpanel">
                <div class="_14d25">
                  ...
                  <table class="table table-bordered ucp-table">
                    ...
                    <tbody>
                    @if(count($electiveBatch->assignments) > 0)
                    @foreach($electiveBatch->assignments as $assignmentKey => $assignment)
                      <tr>
                        <td>{{ $assignmentKey+1 }}</td>
                        <td>{{ $assignment->name }}</td>
                        <td>{{ $electiveBatch->program->name }}</td>
              ...
              <div class="_14d25">
                <div class="row">
                  ...
                  @if($electiveBatch->batchUsers)
                  @foreach($electiveBatch->batchUsers as $userElective)
                  <div class="col-xl-3 col-lg-4 col-md-6">
                    <div class="fcrse_1 mt-30">
                      <div class="tutor_img">
                      <a href="#"><img src="{{ asset('storage/users/'.$userElective->user->photo) }}" alt=""></a>												
                      </div>
                      <div class="tutor_content_dt">
                        <div class="tutor150">
                        <a href="#" class="tutor_name">{{ $userElective->user->first_name.' '.$userElective->user->last_name }}</a>
                        </div>
                        <div class="tutor_cate">{{ $userElective->user->email }}</div>
                        <ul class="tutor_social_links">
                          @if($userElective->user->facebook_profile_url)
                          <li><a href="{{ $userElective->user->facebook_profile_url }}" class="fb"><i class="fab fa-facebook-f"></i></a></li>
                          @endif
                          @if($userElective->user->twitter_profile_url)
                          <li><a href="{{ $userElective->user->twitter_profile_url }}" class="tw"><i class="fab fa-twitter"></i></a></li>
                          @endif
                          @if($userElective->user->linkedin_profile_url)
                          <li><a href="{{ $userElective->user->linkedin_profile_url }}" class="ln"><i class="fab fa-linkedin-in"></i></a></li>
                          @endif
                          @if($userElective->user->instagram_profile_url)
                          <li><a href="{{ $userElective->user->instagram_profile_url }}" class="yu"><i class="fab fa-instagram"></i></a></li>
                          @endif
                        </ul>
                        <div class="tut1250">
                          <span class="vdt15">{{ $userElective->user->activeProgramsCount() }} Programs</span>
                          <span class="vdt15">{{ $userElective->user->activeElectivesCount() }} Electives</span>
                        </div>
                      </div>
                    </div>										
                  </div>
              ...
              <div class="tab-pane fade {{ $activeTab == 'feedback' ? 'show active' : '' }}" id="nav-comments-feedback" role="tabpanel">
                ...
                @if($feedbacksCount < 3 && $electiveBatch->end_date < Carbon\Carbon::today())
                  <h3>Share Your Feedback</h3>

                  <form method="post" action="{{ route(config('app.p_slug').'.my_electives.batches.feedback.store', [$electiveBatch->program->id, $electiveBatch->id]) }}">
                  @csrf
...
{{ Form::open(array('route' => [Config::get('app.p_slug').'.my_electives.batches.submit_assignment', [$electiveBatch->program->id, $electiveBatch->id, $assignment->id]], 'method' => 'post', 'files' => true)) }}
        ...
        <table class="table table-bordered ucp-table">
          <tbody>
            <tr>
              ...
              <td><b>Elective:</b><br>{{ $electiveBatch->program->name }}</td>
            ...
            <tr>
              ...
              <td colspan="2">
                @if($assignment->userElectiveAssignment)
                @if($assignment->userElectiveAssignment->document)
