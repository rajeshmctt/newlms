...
<div class="col-lg-3 col-md-4">
  <div class="fcrse_1 mt-30">
    <a href="{{ route(config('app.p_slug').'.my_electives.batches.show', [$electiveBatch->program->id, $electiveBatch->id]) }}" class="fcrse_img">
      ...
      <div class="course-overlay">
        ...
        @endif
        <div class="crse_timer">{{ $electiveBatch->batchUser->status == 'completed' ? 'Completed' : 'Enrolled' }}</div>
      </div>
    </a>
    <div class="fcrse_content">
      ...
      <a href="{{ route(config('app.p_slug').'.my_electives.batches.show', [$electiveBatch->program->id, $electiveBatch->id]) }}" class="crse14s"
        >{{ $electiveBatch->program->name }}
        </a
      >
      <a href="{{ route(config('app.p_slug').'.my_electives.batches.show', [$electiveBatch->program->id, $electiveBatch->id]) }}" class="crse-cate">
        {!! Str::limit($electiveBatch->program->description, 150) !!}</a>
      <div class="auth1lnkprce">
        <p class="cr1fot">By 
          
          @if($electiveBatch->program->faculties)
          @foreach($electiveBatch->program->faculties as $facultyKey => $faculty)
        ...
        <div class="prce142">
          <a href="{{ route(config('app.p_slug').'.my_electives.batches.show', [$electiveBatch->program->id, $electiveBatch->id]) }}">
